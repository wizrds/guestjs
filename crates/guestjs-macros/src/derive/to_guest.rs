...
impl ToGuestDerive {
    ...
    pub(crate) fn expand(self) -> Result<TokenStream, syn::Error> {
        ...
        bound_generics.params.insert(
            0,
            GenericParam::Lifetime(LifetimeParam::new(bound_lifetime.clone())),
        );
...
mod tests {
    ...
    fn accepts_structs_and_enums() {
        assert!(ToGuestDerive::new(&parse_quote!(struct Record;)).is_ok());
        assert!(ToGuestDerive::new(&parse_quote!(enum State { Ready })).is_ok());
    }
    ...
    fn generates_owned_and_bound_implementations() {
        ...
        assert!(
            output.contains(
                "crate :: marshal :: ToGuestBound < '__guestjs > for State",
            ),
        );
        assert_eq!(
            output
